...
url_pdf_validar = 'http://127.0.0.1:5000/servicoELIS/Arquivo/Processar'


file_path = "D:\\ELIS_2\\"

filename = "20190905144937_TJPEReports_Completo_filtrado.csv"

filename_resultado = "temp_resultado_primeira_quinzena_setembro.csv"
file_path_csv = os.path.join(file_path, filename)
file_path_csv_resultado = os.path.join(file_path, filename_resultado)
file_path_cda = os.path.join(file_path, 'ConsultaCDA_Jan_2019\consultacda\processos\documentos')
...
with open(file_path_csv) as csv_file:
    ...
    npu_cda_vazias = []
    npu_cda_invalidas = []
    unidade = "ExecutivosFiscaisRecife"
        ...
        if line_count > 0:
            ...
            if (not arquivo_vazio) and (not arquivo_inexistente):
                ...
                if not arquivo_valido:
                    ...
                    #cda_sem_pontuacao = str(j["CDA_SEM_PONTUACAO"])
                        data = '''{
                            "unidade": "''' + unidade + '''",
                            "dadosPje": {
                                "NPU" :  "''' + npu + '''",
                        ...
                        if response_final.status_code == 200:
                            ...
                            npu_resultados = str(json_resultado_modelos["npu"])
                            cda_resultados = json_resultado_modelos["cda"]
