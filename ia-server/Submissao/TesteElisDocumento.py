                ...
                if not arquivo_valido:
                    ...
                    if sucesso_envio_cda:
                        try:
                            ...
                            j = json.loads(response.content)
                            # Junta e submete ao modelo definitivo
                            cda_doc = str(j["CDA_CPF_CNPJ"])

                            doc_cda_pje_igual = 0
                            temp_cda_doc = cda_doc.replace(".", "").replace(",", "").replace("-", "").replace("/", "")
                            temp_pje_doc = cpf_cnpj_executado.replace(".", "").replace(",", "").replace("-", "").replace("/", "")
                            if temp_cda_doc == temp_pje_doc:
                                doc_cda_pje_igual = 1

                            #npu_resultados = str(chaves["NPU"])
                            #cda_resultados = str(chaves["CDA"])

                            print(npu + "|" + cda + "|CDA_DOC: " + cda_doc +
                                  "|PJE_DOC: " + cpf_cnpj_executado + "|IGUAL: " + str(doc_cda_pje_igual))
